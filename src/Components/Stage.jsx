        ...
        {stages?.map((stage, i) =>
          ...
          className='stage flex flex-col w-full border-2 rounded-2xl bg-white h-150 p-3 px-4 gap-4'>
            <div className='flex justify-between w-full items-center'>
              ...
              {stage.items.length < 10 ? <button onClick={() => handleCreateItemOpenClose(stage.stage_id)} className='bg-blue-400 px-2 p-1 rounded-md w-fit cursor-pointer'>+</button> : null}
              <button onClick={()=>handleRemoveStage(stage.stage_id)} className='bg-red-400 px-2 p-1 rounded-md w-fit cursor-pointer'>x</button>
              </div>
            </div>
            ...
            <div className='flex flex-col gap-3 overflow-auto scrollbar-hidden h-[85%]'>
              {stage?.items?.map((item) =>
                <ul
                  ...
                  <li className='absolute top-0 right-3'><button onClick={()=>handleRemoveItem(stage.stage_id , item.item_id)} className='text-lg cursor-pointer'>x</button></li>
